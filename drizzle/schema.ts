...
export const users = mysqlTable("users", {
  ...
  sparkBalance: int("sparkBalance").default(100).notNull(), // 新用戶送 100 Spark
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  lastSignedIn: timestamp("lastSignedIn"),
  subdomain: varchar("subdomain", { length: 63 }).unique(), // Free subdomain: xxx.lulubaby.xyz
  // === 推薦系統字段 ===
  referralCode: varchar("referralCode", { length: 16 }).unique(), // 格式 LULU-XXXXXX
  referredById: int("referredById"), // 推薦人的 user ID
});
...
/**
 * Chat sessions - groups conversation messages
 */
export const chatSessions = mysqlTable("chat_sessions", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  sessionId: varchar("sessionId", { length: 64 }).notNull().unique(),
  title: varchar("title", { length: 255 }).default("新對話").notNull(),
...
export const aiTraining = mysqlTable("ai_training", {
  ...
  // 📡 主動資料索取 (Proactive Data Collection)
  proactiveDataCollection: boolean("proactiveDataCollection").default(false).notNull(), // 是否主動索取客戶資料
...
/**
 * Subscription plans for users - Stripe integrated
 * 用戶訂閱計劃 - Stripe 支付整合
 */
...
export const PLAN_LIMITS = {
  free: {
    ...
    knowledgeBaseCharsLimit: 50000, // 免費用戶知識庫注入上限 50K 字元
  ...
  pro: {
    ...
    knowledgeBaseCharsLimit: 200000, // Pro 用戶知識庫注入上限 200K 字元
  ...
  enterprise: {
    ...
    knowledgeBaseCharsLimit: 500000, // 企業用戶知識庫注入上限 500K 字元
...
export type InsertSparkTransaction = typeof sparkTransactions.$inferInsert;
...
export const SPARK_PACKAGES = {
  ...
  energy: { name: "能量包", nameEn: "Energy Pack", price: 288, currency: "HKD", sparks: 3000, bonus: 500, tagline: "最受歡迎的選擇" },
  super: { name: "超級包", nameEn: "Super Pack", price: 888, currency: "HKD", sparks: 10000, bonus: 2000, tagline: "釋放全部潛能" },
  unlimited: { name: "無限包", nameEn: "Unlimited Pack", price: 2888, currency: "HKD", sparks: 40000, bonus: 10000, tagline: "企業級用量" },
} as const;
...
export const SPARK_COSTS = {
  chatMessage: 1,
  knowledgeBasePerMB: 10,
  knowledgeBaseOverlimitPerChat: 3,
  
  superpowerActivation: 30,
} as const;
...
/**
 * Widget settings for each user/persona
 * 每個用戶/AI角色的Widget設定
 */
export const widgetSettings = mysqlTable("widget_settings", {
  id: int("id").autoincrement().primaryKey(),
  personaId: int("personaId").notNull().unique(),

  // 外觀設定
  widgetEnabled: boolean("widgetEnabled").default(true).notNull(),
  position: varchar("position", { length: 20 }).default("bottom-right").notNull(),
  size: varchar("size", { length: 20 }).default("medium").notNull(),
  bubbleSize: int("bubbleSize").default(60).notNull(),
  showBubbleText: boolean("showBubbleText").default(true).notNull(),
  bubbleText: varchar("bubbleText", { length: 50 }).default("需要幫助嗎？"),

  // 行為設定
  autoOpen: boolean("autoOpen").default(false).notNull(),
  autoOpenDelay: int("autoOpenDelay").default(5).notNull(),
...
export type InsertWidgetSetting = typeof widgetSettings.$inferInsert;
