...
describe("Superpowers API", () => {
  ...
  describe("superpowers.update", () => {
    it("should update superpower toggles", async () => {
      ...
      // Ensure user has enough Spark balance for superpowers
      const { addSpark } = await import('./db');
      await addSpark(ctx.user!.id, 5000, 'Test topup for superpowers');
    ...
    it("should toggle superpowers on and off", async () => {
      ...
      // Ensure enough Spark
      const { addSpark: addSpark2 } = await import('./db');
      await addSpark2(ctx.user!.id, 500, 'Test topup for toggle');
      ...
      // Turn off (no cost for disabling)
    ...
    it("should update multiple categories at once", async () => {
      ...
      // Ensure enough Spark
      const { addSpark: addSpark3 } = await import('./db');
      await addSpark3(ctx.user!.id, 5000, 'Test topup for multi-category');
