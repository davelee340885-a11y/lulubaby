  ...
  async getMemoryContext(conversationContext: string, limit: number = 5): Promise<string> {
    ...
    if (memories.length === 0) {
      ...
      if (recentImportant.length === 0) {
        // 第三層 fallback：獲取最近的所有記憶（不限重要性）
        const recentAny = await this.getRecentMemories(limit);
        if (recentAny.length === 0) {
          return "";
        }
        relevantMemories = recentAny.map((entry: LearningDiaryEntry) => ({
          entry,
          relevanceScore: 0.3,
...
【最高優先級】你已知的客戶資訊和專業知識
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
以下是用戶已記錄的客戶資訊和專業知識，你必須優先使用這些資訊回答問題：\n`;
    ...
    contextPrompt += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【強制指令】
1. 當用戶詢問任何與以上客戶或知識相關的問題時，你必須直接引用以上資訊回答
2. 禁止說「我不知道」、「我沒有這個記錄」、「請查閱您的記錄」
3. 回答格式範例：「根據我的記錄，張先生對醫療保險很感興趣，他有兩個小孩，也關心教育基金...」
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  ...
  /**
   * 獲取最近的所有記憶（不限重要性，作為最後的 fallback）
   */
  private async getRecentMemories(limit: number = 5): Promise<LearningDiaryEntry[]> {
    const db = await getDb();
    if (!db) return [];
